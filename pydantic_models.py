...
class Multiple_PV_Actual(BaseModel):
    ...
    actual_values: list ([One_PV_Actual]) =  Field(..., description="Actual values for multiple sites") 
...
class Site_Forecast_Values(BaseModel):
  ...
  expected_generation_kw: str = Field(..., description="Expected generation in kw", ge=0)

# get_forecast
#client gets a forecast for their site
class Forecast(BaseModel):
  forecast_uuid: str = Field(..., description="The forecast id")
  forecast_values: list(Site_Forecast_Values) = Field(..., description="List of target times and generation")

# client gets information about the forecast: which site, which forecast version, when it was made
# parameters: site_uuid
class Forecast_Metadata(EnhancedBaseModel):
