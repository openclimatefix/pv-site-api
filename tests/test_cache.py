...
def test_remove_old_cache(caplog):
    with caplog.at_level(logging.DEBUG):
        ...
        last_updated = {
            'key1': now - timedelta(seconds=160),  # Older than 120 seconds
            'key2': now - timedelta(seconds=180),  # Older than 120 seconds
            'key3': now - timedelta(seconds=60),   # Within 120 seconds
        }
